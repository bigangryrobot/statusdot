DashboardPublic = React.createClass({
  mixins: [ ReactMeteorData ],
  getMeteorData() {
    let query = {};
    Meteor.subscribe( 'environmentsPublic' );
    ...
    return {
      environments: Environments.find({ isPublic: true }).fetch(),
  ...
  renderComponentsDashboard() {
    if ( this.data.components.length > 0 ) {
      ...
      return <ComponentDashboard items={ this.data.components } environments={ this.data.environments} />;
    } else {
